    ...
    if routes:
        ...
        for r in routes[:10]:
            ...
            lines.append(
                ...
                f"| {r.time_hm} | {cc} | {fl} | {r.schedule_label} |"
            )
        lines += [
            "",
            "- **コース定数**＝路線的體力負荷總量（依行動時間、距離、爬升/下降計算）：",
            "  〜19 輕鬆、20〜39 一般、40〜59 健腳、60〜79 吃力、80〜 極吃力。",
            "- **体力度**（1〜10）＝Yamap 依コース定数換算的分級：",
            "  1〜3 適合日帰り、4〜5 建議住 1 晚以上、6〜10 需多日縱走的體力。",
            "- **日程**＝Yamap 建議的行程天數；標準時間不含休息。",
            "",
        ]
