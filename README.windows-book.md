...
新版已明確使用 UTF-8 讀寫住客資料，並設定 Windows 終端機輸出編碼。
下方仍保留 `PYTHONUTF8` 設定，讓舊版及其他 Python 元件也使用 UTF-8；它只對目前 PowerShell 視窗有效。
...
- 若需指定房型，可在最後加上 `--room "2名様"`，文字必須符合網站的房型名稱。
  不指定時會依 `-m`＋`-w` 篩選「N名様」利用人數，等待開賣後亦同；最後仍需核對房型與人數。
...
- **`UnicodeEncodeError: cp950`**：先在同一個 PowerShell 執行
  `$env:PYTHONUTF8 = "1"`，再重新執行原命令。如果錯在 `book --setup`，
  請檢查 `booking_profile.json` 是否為空檔；新版重跑 `--setup` 會自動重建空白範本，
  已填寫的非空白資料不會被覆蓋。
