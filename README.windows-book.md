...
**目前程式使用電腦本地時間，沒有自動把日本開賣時間換算成台灣時間。**
請在啟動 `book` 前，到 Windows「設定 → 時間與語言 → 日期與時間」：

1. 如有開啟「自動設定時區」，先關閉。
2. 時區選擇 **大阪、札幌、東京（UTC+09:00）**。
3. 保持自動設定時間，並執行時間同步。

不要手動把時鐘加一小時；更改的是「時區」。搶位結束後可以改回台北（UTC+08:00）。

例如官網公告日本 **08:00** 開賣，就是台灣 **07:00**。
當你在台灣 06:50 準備啟動時，已切換日本時區的電腦應顯示 07:50。
...
- **`Invalid format string`**：目前過早啟動（距開賣超過 12 小時）的提示使用了
  Windows 不支援的日期格式。請在確認的開賣前約 10 分鐘再執行；若仍出錯，需修正程式。
